...
class RobotSLAMTrn(Node):
    def __init__(self , target, axis, speed= 25, stop_limit = 100):#, target):
        ...
        self.cmd_input_sub = self.create_subscription(RobotCommand, self.cmd_input_topic, self.cmd_callback, 10)
    ...
    def cmd_callback(self, cmd:RobotCommand):
        pose = RobotPose()
        try:
            """
            mode definition: 
            ...
            0.0 >> Stop
            """
            rec_mov_mode = cmd.name 
            rec_com = round(cmd.command, 1)
            speed = cmd.speed
            robot_pose = self.robot_pose

            if rec_mov_mode == 'w_target':
                    pose.mode = rec_com
                    pose.speed = speed
                    self.cmd_pub.publish(pose)
                    rclpy.logging.get_logger("move Forward").info(f'mode: {rec_mov_mode} / {str(robot_pose)}')
            
            
            if rec_mov_mode == 'target':
                if self.axis == 'z':
